        ...
        <template #title>
            ...
            <div class="flex justify-between items-start py-2 px-6 rounded-t border-b dark:border-gray-600">
                <h3 class="text-lg mt-2 font-semibold text-gray-900 dark:text-white">
                    {{ __("list_of_permissions_title") }} "{{ module_data.name}}"
                </h3>
                    ...
                    <div class="w-6xl py-2 px-2 lg:px-2">
                        <div class="relative overflow-x-auto shadow-sm sm:rounded-lg">
                            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    <tr v-for="permission in permissionsOptions" :key="permission" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        <td class="w-16 p-1">
                                            <div class="flex items-center">
    ...
    export default defineComponent({
        ...
        watch: {
            module_data: {
                handler(permission){
                    ...
                    const array = []
                    permission.permissions.map(function(value) {
                        array.push(value.name.split(".")[1]);
                    });
        ...
        methods : {
            checkAll: function(){
                ...
                if(this.isCheckAll){
                    this.permissionsOptions.map(function(value) {
                        array.push(value);
                    });
                }
            ...
            submit(){
                ...
                this.form = this.$inertia.form({
                    _method: 'put',
                    module_id: this.module_data.id,
                    permissions: this.permissions_values
                })

                this.form.post(route("permissions.update", this.module_data.id), {
                    preserveState: true,
                    onSuccess: () => {
                        this.closeModal(),
                        this.errors.permissions = ""
